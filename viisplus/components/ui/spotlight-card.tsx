...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const getInlineStyles = (): React.CSSProperties & Record<string, string | number> => {
    const styles: React.CSSProperties & Record<string, string | number> = {
      ...
      '--radius': '14',
      '--border': '1',
      '--backdrop': 'hsl(174 90% 5% / 0.7)',
      '--backup-border': 'hsl(174 40% 20% / 0.5)',
      '--size': '200',
      '--outer': '1',
      ...
      backgroundImage: `radial-gradient(
        ...
        calc(var(--x, 0) * 1px)
        calc(var(--y, 0) * 1px),
        hsl(var(--hue, 174) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 70) * 1%) / var(--bg-spot-opacity, 0.08)), transparent
      )`,
  ...
  const css = `
    ...
    [data-glow]::before {
      background-image: radial-gradient(
        ...
        hsl(var(--hue, 174) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 50) * 1%) / var(--border-spot-opacity, 1)), transparent 100%
      );
      filter: brightness(2);
    }
    [data-glow]::after {
    ...
    <>
      ...
      <div
        ...
        className={`
          ...
          shadow-[0_1rem_2rem_-1rem_black] p-6 gap-4
          backdrop-blur-[5px]
          ${className}
        `}
      >
